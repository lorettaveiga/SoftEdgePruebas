...
const ModificationHistory = ({ projectId }) => {
      ...
      if (Array.isArray(value) && ["EP", "RF", "RNF", "HU"].includes(key)) {
        value.forEach(change => {
          ...
          if (change.changes.nuevo) {
            items.push(<li key={key+id+"nuevo"}>Se agregó {getRequirementTypeSingular(key)} con ID <b>{id}</b></li>);
          } else if (change.changes.eliminado) {
            items.push(<li key={key+id+"eliminado"}>Se eliminó {getRequirementTypeSingular(key)} con ID <b>{id}</b></li>);
          } else {
            Object.entries(change.changes).forEach(([field, vals]) => {
              const fieldLabel = field === 'data' ? 'descripción' : field === 'titulo' ? 'título' : field;
              items.push(<li key={key+id+field}>En {getRequirementTypeSingular(key)} <b>{id}</b>, se cambió <b>{fieldLabel}</b> de "{vals.oldValue}" a "{vals.newValue}"</li>);
            });
          }
        });
      } else {
        switch(key) {
          case 'nombreProyecto':
            items.push(<li key={key}>Se cambió el nombre del proyecto de "{value.oldValue}" a "{value.newValue}"</li>);
            break;
          case 'descripcion':
            items.push(<li key={key}>Se modificó la descripción del proyecto</li>);
            break;
          case 'EP':
          case 'RF':
          case 'RNF':
          case 'HU':
            items.push(<li key={key}>Se modificaron los {getRequirementType(key)}</li>);
            break;
          default:
  ...
  if (history.length === 0) {
    return <div className="modification-history">No hay modificaciones registradas</div>;
  }
        ...
        {sortedHistory.map((item, index) => (
          <div key={index} className="history-item">
            ...
            <div className="history-user">
              ...
              <span className="user-name">{item.userName ? `${item.userName} ${item.userLastname || ''}`.trim() : item.userId}</span>
            </div>
          </div>
        ))}
      </div>
